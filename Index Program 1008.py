...
# Rule 7 (for male): index year = the year obtained the Xiucai - 21
    for i in df.index:
        if index_year_dict[df['c_personid'][i]] == -9999: # check whether the index year has been already calculated
            if df['c_entry_code'][i] == 29: # Jinshi (added)
                index_year_dict[df['c_personid'][i]] = (df['c_year'][i]-51)
                trace_dict[df['c_personid'][i]] = "Rule 5."
            elif df['c_entry_code'][i] in [36,37,44,124,165]: # Jinshi
                index_year_dict[df['c_personid'][i]] = (df['c_year'][i]-30)
                trace_dict[df['c_personid'][i]] = "Rule 5."
            elif df['c_entry_code'][i] in [34,39,46,179,185,186]: # Juren
                index_year_dict[df['c_personid'][i]] = (df['c_year'][i]-27)
                trace_dict[df['c_personid'][i]] = "Rule 6."
            elif df['c_entry_code'][i] in [47,173,257]: # Xiucai
...
# Rule 10: index year = youngest elder brother's birth year + 2
    dic = {} # create an empty dictionary
    for i in personid_list:
        dic[i] = []
    for i in df.index:
        if index_year_dict[df['c_personid'][i]] == -9999: # check whether the index year has been already calculated
            if df['c_kin_code'][i] == 125 or df['c_kin_code'][i] == 165: # elder brother
                if df['c_kin_id'][i] in birthyear_dict: # check whether elder brother's birth year is known
                    dic[df['c_personid'][i]].append(df['c_kin_id'][i])
    for key in dic:
        if dic[key] != []: # check whether the person have any elder brothers
            birthyear_list = []
            for i in range (len(dic[key])):
                birthyear_list.append(birthyear_dict[dic[key][i]])
            youngest_birthyear = max(birthyear_list) # get the max value of all birth years = youngest elder brother
            index_year_dict[key] = (youngest_birthyear+2)
            trace_dict[key] = f"{dic[key][birthyear_list.index(max(birthyear_list))]}, Rule 10."
            
def rule11(df): # kinship data will be used
# Rule 11: index year = oldest younger brother's birth year - 2
    dic = {} # create an empty dictionary
    for i in personid_list:
        dic[i] = []
    for i in df.index:
        if index_year_dict[df['c_personid'][i]] == -9999: # check whether the index year has been already calculated
            if df['c_kin_code'][i] == 126 or df['c_kin_code'][i] == 166: # younger brother
                if df['c_kin_id'][i] in birthyear_dict: # check whether younger brother's birth year is known
                    dic[df['c_personid'][i]].append(df['c_kin_id'][i])
    for key in dic:
        if dic[key] != []: # check whether the person have any younger brothers
            birthyear_list = []
            for i in range (len(dic[key])):
                birthyear_list.append(birthyear_dict[dic[key][i]])
            oldest_birthyear = min(birthyear_list) # get the min value of all birth years = oldest younger brother
            index_year_dict[key] = (oldest_birthyear-2)
            trace_dict[key] = f"{dic[key][birthyear_list.index(min(birthyear_list))]}, Rule 11."
...
# Rule 12: 
    ...
    for i in df.index:
        if index_year_dict[df['c_personid'][i]] == -9999: # check whether the index year has been already calculated
            if df['c_kin_code'][i] == 181 or df['c_kin_code'][i] == 201 or df['c_kin_code'][i] == 224 or df['c_kin_code'][i] == 332: # son-in-law (first daughter's husband)
                if df['c_kin_id'][i] in birthyear_dict: # check whether son-in-law's birth year is known
                    dic[df['c_personid'][i]].append(df['c_kin_id'][i])
    for key in dic:
        if dic[key] != []: # check whether the person have any son-in-laws
            birthyear_list = []
            for i in range (len(dic[key])):
                birthyear_list.append(birthyear_dict[dic[key][i]])
            oldest_birthyear = min(birthyear_list) # get the min value of all birth years = oldest son-in-law
            if gender_dict[key] == 'male': # male
                index_year_dict[key] = (oldest_birthyear-27)
                trace_dict[key] = f"{dic[key][birthyear_list.index(min(birthyear_list))]}, Rule 12."
            elif gender_dict[key] == 'female': # female
                index_year_dict[key] = (oldest_birthyear-24)
                trace_dict[key] = f"{dic[key][birthyear_list.index(min(birthyear_list))]}, Rule 12."
...
# Rule 16: index year = youngest elder brother's index year + 2
    dic = {} # create an empty dictionary
    for i in personid_list:
        dic[i] = []
    for i in df.index:
    ...
    for key in dic:
        if dic[key] != []: # check whether the person have any elder brothers
            index_year_list = []
            for i in range (len(dic[key])):
                index_year_list.append(index_year_dict[dic[key][i]])                
            youngest_index_year = max(index_year_list) # get the max value of all index years = youngest elder brother
            index_year_dict[key] = (youngest_index_year+2)
            trace_dict[key] = f"{dic[key][index_year_list.index(max(index_year_list))]}, Rule 16."

def rule17(df): # kinship data will be used
# Rule 17: index year = oldest younger brother's index year - 2
    dic = {} # create an empty dictionary
    for i in personid_list:
        dic[i] = []
    for i in df.index:
    ...
    for key in dic:
        if dic[key] != []: # check whether the person have any younger brothers
            ...
            for i in range (len(dic[key])):
                index_year_list.append(index_year_dict[dic[key][i]])
            oldest_index_year = min(index_year_list) # get the min value of all index years = oldest younger brother
            index_year_dict[key] = (oldest_index_year-2)
            trace_dict[key] = f"{dic[key][index_year_list.index(min(index_year_list))]}, Rule 17."
...
# Rule 18:
    ...
    for key in dic:
        if dic[key] != []: # check whether the person have any son-in-laws
            ...
            for i in range (len(dic[key])):
                index_year_list.append(index_year_dict[dic[key][i]])
            oldest_index_year = min(index_year_list) # get the min value of all index years = oldest son-in-law
            if gender_dict[key] == 'male': # male
                index_year_dict[key] = (oldest_index_year-27)
                trace_dict[key] = f"{dic[key][index_year_list.index(min(index_year_list))]}, Rule 18."
            elif gender_dict[key] == 'female': # female
                index_year_dict[key] = (oldest_index_year-24)
                trace_dict[key] = f"{dic[key][index_year_list.index(min(index_year_list))]}, Rule 18."
